...
class ImageViewer(object):
    ...
    * 'ESC' : quit
    ...
    def run(self, update_fun=None, path=None):
        ...
        dem = 0
        img_m = []
        #self.out = cv2.VideoWriter('saved_frames/sad.avi',cv2.VideoWriter_fourcc(*'DIVX'), 15, (640,480))
        ...
        while not self._terminate:
          if dem < 13501:
            ...
            if not is_paused:
              ...
              #self.img = cv2.resize(self.image, self._window_shape[:2])
              #self.out.write(self.img)
                #ff_name = "{}{}".format('frame') + str(dem).zfill(5) + '.jpg'
                ff_name = "test" + str(dem).zfill(5) + ".jpg"
            print('ffname',os.path.join(path_save, ff_name))
            cv2.imwrite(os.path.join(path_save, ff_name), self.image)
            #print(os.path.join(path_save,ff_name))
            #img_m.append(self.image)
            dem += 1
          else:
            break
        

        # self.out = cv2.VideoWriter('/content/sad.avi',cv2.VideoWriter_fourcc(*'DIVX'), 15, (640,480))
        # for k in range(len(img_m)):
        #   self.out.write(img_m[k])
            # cv2.imshow(
            #     self._caption, )
